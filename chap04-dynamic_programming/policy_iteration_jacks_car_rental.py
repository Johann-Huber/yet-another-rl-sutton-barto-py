                ...
                for num_ret_1 in range(MAX_RANGE_REQ_RET):
                    for num_req_2 in range(MAX_RANGE_REQ_RET):
                        for num_ret_2 in range(MAX_RANGE_REQ_RET):
                            ...
                            next_state, reward = car_rental.run_step(state, action, n_car_request, n_car_returned)
...
def run_policy_iteration_jacks_car_rental():
    """Reproduces figure p.81"""

    car_rental = CarRental()
    policy = Policy(fix_returned=True, env=car_rental)
...
if __name__ == '__main__':
    run_policy_iteration_jacks_car_rental()